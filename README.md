...
For a given file, if one branch has modifications, the changes in that branch will be retained. If both branches have modifications and the content is the same, it will still be retained. These are all standard situations, and the generated new commit message is 'Merged [given branch name] into [current branch name]'.

However, if both branches have different modifications to the same file, Gitlet will encounter a conflict, not knowing which version to keep. Upon a conflict, it will first display 'Encountered a merge conflict.' Then, it will generate a conflicted file with the following format:

![image](https://github.com/xingyeahhh/Gitlet/assets/123461462/bc26e86e-ecb5-4c35-8862-e1c58f94e01e)
